...
void tabulate(void)
{
    for (int voter = 0; voter < voter_count; voter++)
    {
        ...
        {
            if (candidates[preferences[voter][count]].eliminated != true)
            {
                candidates[preferences[voter][count]].votes++;
                break;
            }
        }
    }
...
bool print_winner(void)
{
    ...
    {
        if (candidates[win].votes > winningVote)
        {
            printf("%s\n", candidates[win].name);
            return true;
        }
...
bool is_tie(int min)
{
    ...
    {
        if (candidates[tie].votes != min && candidates[tie].eliminated != true)
        {
            return false;
        }
    }
